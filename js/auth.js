...
if (register) {
  register.addEventListener("submit", async (e) => {
    ...
    const graduation_year = Number(
      document.querySelector("#graduation_year").value,
    );
    ...
    if (!first_name || !last_name)
      return show("Please enter your first and last name.", true);
    if (
      !Number.isInteger(graduation_year) ||
      graduation_year < GRAD_MIN ||
      graduation_year > GRAD_MAX
    )
      return show(
        `Please enter a graduation year between ${GRAD_MIN} and ${GRAD_MAX}.`,
        true,
      );
    if (!document.querySelector("#consent").checked)
      return show(
        "Please authorize your profile to be included in the private directory.",
        true,
      );
    ...
    register.reset();
    if (data.session)
      show("Account created. Your profile is pending administrator approval.");
    else
      show(
        "Account created. Check your email to verify your address. Your profile will appear after administrator approval.",
      );
  });
}
...
if (forgotToggle && resetRequest) {
  forgotToggle.addEventListener("click", (e) => {
    ...
    forgotToggle.textContent = resetRequest.hidden
      ? "Forgot your password?"
      : "Back to login";
  });
  const resetMsg = document.querySelector("#reset-message");
  function showReset(text, error = false) {
    if (resetMsg) {
      resetMsg.textContent = text;
      resetMsg.className = "message " + (error ? "error" : "success");
    }
  }
