...
function render() {
  ...
  results.innerHTML =
    ...
    <article class="card person">
      <h3>${esc(r.first_name)} ${esc(r.last_name)}</h3><div class="muted">Class of ${r.graduation_year}</div>
      ${r.city || r.state ? `<p>${esc(r.city || "")}${r.city && r.state ? ", " : ""}${esc(r.state || "")}</p>` : ""}
      ${r.street_address ? `<p>${esc(r.street_address)}<br>${esc(r.city || "")}, ${esc(r.state || "")} ${esc(r.zip || "")}</p>` : ""}
      ${r.phone ? `<p>☎ ${esc(r.phone)}</p>` : ""}${r.email ? `<p>✉ ${esc(r.email)}</p>` : ""}
      ${r.allow_messages ? `<a class="button button-small" href="contact.html?id=${encodeURIComponent(r.id)}">Contact</a>` : ""}
    </article>`,
      )
      .join("") || `<div class="card"><p>No alumni found.</p></div>`;
}
function esc(s) {
...
document.querySelector("#year").oninput = render;
init();
