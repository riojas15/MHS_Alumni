...
async function init(){
 ...
 const {data,error}=await supabase.from("profiles").select("id,first_name,last_name,graduation_year,street_address,city,state,zip,phone,show_address,show_phone,show_email,allow_messages,status").eq("id",user.id).single();
